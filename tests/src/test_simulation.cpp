  ...
  GIVEN("Two replicas with different num staples") {
    ...
    WHEN("PTMC exchange probability is calculated") {
      ...
      if (sim.m_rank == 0) {
        ...
        double enthalpy1{
            origami.hybridization_enthalpy(scaffold_d_1, staple1_d_1) +
            origami.hybridization_enthalpy(scaffold_d_2, staple1_d_2)};
        double enthalpy2{0};
        ...
        double p_accept{
            sim.calc_acceptance_p(control_q_pairs, dependent_q_pairs)};

        THEN("Value matches expected") { REQUIRE(p_accept == exp_p_accept); }
      }
    }
