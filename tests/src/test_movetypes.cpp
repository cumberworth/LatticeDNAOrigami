...
template <typename T>
bool compare_approx_vector_contents(vector<T> first_vector,
...
SCENARIO("Example moves work as expected") {
  ...
  WHEN("CB staple regrowth move attempted") {
    ...
    // Setup movetype
    CBStapleRegrowthMCMovetype movetype;
    // {origami, random_gens,
    ...
    double delta_e{
        origami.set_domain_config(staple1_d_2, {0, 0, 0}, {0, 0, -1})};
    ...
    double delta_e_alt =
        origami.hybridization_energy(staple1_d_1, scaffold_d_2);
  ...
  WHEN("CTCB scaffold regrowth move attempted") {
    ...
    vector<Domain *> scaffold_domains{&scaffold_d_2, &scaffold_d_3,
                                      &scaffold_d_4};
    ...
    movetype.calc_bias(bfactors, &scaffold_d_3, configs, p_prev,
                       scaffold_domains);
    origami.set_domain_config(scaffold_d_3, {1, 1, 0}, {0, 1, 0});
    ...
    movetype.calc_bias(bfactors, &scaffold_d_4, configs, p_prev,
                       scaffold_domains);
    origami.set_domain_config(scaffold_d_4, {0, 1, 0}, {0, -1, 0});
    movetype.m_constraintpoints.update_endpoints(&scaffold_d_4);

    // Calculate expected
    double delta_e_sd2s2d2{
        origami.hybridization_energy(scaffold_d_2, staple2_d_2)};
    double delta_e_sd3s2d1{
        origami.hybridization_energy(scaffold_d_3, staple2_d_1)};
    double delta_e_sd4s1d2{
        origami.hybridization_energy(scaffold_d_4, staple1_d_2)};
    long double expected_new_bias{exp(-delta_e_sd2s2d2) * 5 * 6 *
                                  exp(-delta_e_sd3s2d1) / 2 *
                                  exp(-delta_e_sd4s1d2)};
...
SCENARIO("Connector staples are correctly identified") {
  ...
  IdentityMCMovetype movetype{origami, random_gens, ideal_random_walks, params};
  ...
  WHEN("Case 4: One staple bound to two others which are both bound to "
       "scaffold") {
    origami.set_domain_config(staple11_d_1, {1, 1, 0}, {0, -1, 0});
    origami.set_domain_config(staple11_d_2, {2, 1, 0}, {0, 1, 0});
    origami.set_domain_config(staple12_d_1, {2, 1, 0}, {0, -1, 0});
...
SCENARIO("Moves are fully run and origami system reset each time") {
  ...
  WHEN("Met staple exchanges are carried out") {
    for (int i{0}; i != 10; i++) {
      MetStapleExchangeMCMovetype movetype{origami, random_gens,
                                           ideal_random_walks, params};
