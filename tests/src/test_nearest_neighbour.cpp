...
SCENARIO("Computed free energies are compared to hand calculated values") {
    GIVEN("A set of fully complimentary sequences and hand calculated "
          "energies") {
        vector<string> seqs {
        ...
        THEN("The associated energies match expected") {
            for (size_t i {0}; i != seqs.size(); i++) {
                double hybrid_e {calc_unitless_hybridization_energy(
                        seqs[i], temp, cation_M)};
    ...
    GIVEN("A [cation] not equal to unity and an arbitrary pair of sequences") {
        ...
        /* (((0.2) + (-7.2) + (2 * 2.2)) - 300 * ((-0.0057) + (-0.0014) +
            (-0.0204) + (2 * 0.0069) + 0.368 * 2 * math.log(0.5) / 1000)) *
            4.184 * 1000 / 8.3144598 / 300
        */
        double energy {2.7895932252887095};
        THEN("The associated energies match expected") {
            double hybrid_e {
                    calc_unitless_hybridization_energy(seq, temp, cation_M)};
