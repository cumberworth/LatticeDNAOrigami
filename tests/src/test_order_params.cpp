...
SCENARIO("Order parameters and bias functions") {
  ...
  double staple_u{molarity_to_chempot(staple_M, temp, lattice_site_volume)};
  ...
  GIVEN("Four domain loop system with dist bias") {
    ...
    OrigamiSystemWithBias origami{identities, sequences, configs,
  ...
  GIVEN("Four domain loop system with dist bias") {
    ...
    bias_grid[{0, 0}] = -10;
    bias_grid[{1, 1}] = -5;
    GridBiasFunction *grid_bias_f{origami.get_system_biases()->get_grid_bias()};
    SystemOrderParams *system_order_params{origami.get_system_order_params()};
    OrderParam *num_bound_domains{
        &system_order_params->get_num_bound_domains()};
    ...
    GIVEN("Unbound scaffold") {
      ...
      double delta_e{
          origami.check_domain_constraints(scaffold_d_4, {3, 0, 0}, {0, 0, 0})};
    ...
    GIVEN("Self-bound scaffold") {
      ...
      double expected_bias{-10};
      double delta_e{
          origami.check_domain_constraints(scaffold_d_4, {1, 0, 0}, {0, 0, 0})};
    ...
    GIVEN("Staple bound correctly at one domain and the unbound") {
      ...
      delta_e =
          origami.check_domain_constraints(staple_d_2, {0, 2, 0}, {0, 0, 1});
      REQUIRE(expected_delta_e == delta_e);
