...
#include "LatticeDNAOrigami/test_origami_system.hpp"
...
OrigamiSystem
testOrigamiSystem::setup_two_domain_scaffold_origami(double temp,
                                                     double cation_M) {
  ...
  OrigamiSystem origami{identities, sequences, configs,
...
OrigamiSystem
TestOrigamiSystem::setup_four_domain_scaffold_origami(double temp,
                                                      double cation_M) {
  // Setups up system with no staples and unassigned scaffold
...
OrigamiSystem TestOrigamiSystem::setup_snodin_origami(double temp,
                                                      double cation_M) {
...
SCENARIO("Energies of an origami system are calculated") {
  ...
  GIVEN("An origami system with 2 scaffold domains and one 2 domain staple") {
    ...
    WHEN("Scaffold 1 and staple 1 bind") {
      ...
      double scaffold_staple_1_ene{
          calc_unitless_hybridization_energy(scaffold_seq_1, temp, cation_M)};
    ...
    WHEN("Scaffold 2 and staple 2 bind") {
      // Direct energy calculation with NN
      double scaffold_staple_2_ene{
          calc_unitless_hybridization_energy(scaffold_seq_2, temp, cation_M)};
    ...
    WHEN("Scaffold 1 and scaffold 2 bind") {
      // Direct energy calculation with NN
      vector<string> scaffold_1_2_seqs{
          find_longest_contig_complement(scaffold_seq_1, scaffold_seq_2)};
      double ene_sum{0};
    ...
    WHEN("Staple 1 and staple 2 bind") {
      // Direct energy calculation with NN
      vector<string> staple_1_2_seqs{
          find_longest_contig_complement(staple_seq_1, staple_seq_2)};
      double ene_sum{0};
    ...
    WHEN("Staple 1 and staple 1 bind") {
      // Direct energy calculation with NN
      vector<string> staple_1_1_seqs{
          find_longest_contig_complement(staple_seq_1, staple_seq_1)};
      double ene_sum{0};
      ...
      origami.set_domain_config(staple_d_1, {0, 0, 0}, {1, 0, 0});
      double delta_e{
          origami.set_domain_config(staple2_d_1, {0, 0, 0}, {-1, 0, 0})};
    ...
    WHEN("Staple 2 and staple 2 bind") {
      // Direct energy calculation with NN
      vector<string> staple_2_2_seqs{
          find_longest_contig_complement(staple_seq_2, staple_seq_2)};
      double ene_sum{0};
...
SCENARIO("Helical constraints are checked") {
  ...
  // domain system already ensure that the total number of configuerations is
  // equal to that calculated by hand. This will mainly be to check the junction
  // constraints, but some of the two domain configurations will be checked
  // anyways
  double temp{300};
  double cation_M{1};
  ...
  GIVEN("Origami system with 4 16 nt scaffold domains and 2 2 domain staples") {
    ...
    WHEN("The first staple is bound to it's domains with any other "
         "configuration") {
    ...
    */
    WHEN("Two copies of the second staple are bound to their respective "
         "domains") {
      origami.add_chain(1);
  ...
  GIVEN("Snodin origami system") {
    ...
    WHEN("First three domains of scaffold are in same linear helix") {
      origami.set_domain_config(*origami.get_domain(0, 0), {0, 0, 0},
                                {0, 0, 1});
      origami.set_domain_config(*origami.get_domain(0, 1), {0, 1, 0},
                                {0, 0, -1});
      origami.set_domain_config(*origami.get_domain(0, 2), {0, 2, 0},
                                {0, 0, 1});
      origami.set_domain_config(*origami.get_domain(1, 0), {0, 0, 0},
                                {0, 0, -1});
      origami.set_domain_config(*origami.get_domain(1, 1), {0, 1, 0},
                                {0, 0, 1});
      origami.set_domain_config(*origami.get_domain(2, 1), {0, 2, 0},
                                {0, 0, -1});
    ...
    WHEN("First three domains of scaffold are in same non-linear helix") {
      ...
      origami.set_domain_config(*origami.get_domain(0, 1), {0, 1, 0},
                                {0, 0, -1});
      origami.set_domain_config(*origami.get_domain(0, 2), {1, 1, 0},
                                {0, 0, 1});
      origami.set_domain_config(*origami.get_domain(1, 0), {0, 0, 0},
                                {0, 0, -1});
      origami.set_domain_config(*origami.get_domain(1, 1), {0, 1, 0},
                                {0, 0, 1});
      origami.set_domain_config(*origami.get_domain(2, 1), {1, 1, 0},
                                {0, 0, -1});
