...
int OrderParam::get_param() { return m_param; }

int OrderParam::get_checked_param() { return m_checked_param; }

bool OrderParam::defined() { return m_defined; }

DistOrderParam::DistOrderParam(
        Domain& domain_1,
        Domain& domain_2,
        string label):
        m_domain_1 {domain_1}, m_domain_2 {domain_2} {
...
int DistOrderParam::check_param(
        Domain& domain,
        VectorThree new_pos,
        VectorThree,
        Occupancy state) {
...
int AdjacentSiteOrderParam::check_param(
        Domain& domain,
        VectorThree new_pos,
        VectorThree,
        Occupancy state) {
...
int SumOrderParam::check_param(Domain&, VectorThree, VectorThree, Occupancy) {
    int op_sum {0};
    m_defined = true;
    for (auto param: m_ops) {
...
int NumStaplesOrderParam::check_param(
        Domain&,
        VectorThree,
        VectorThree,
        Occupancy) {
...
int NumStaplesTypeOrderParam::check_param(
        Domain&,
        VectorThree,
        VectorThree,
        Occupancy) {
...
int StapleTypeFullyBoundOrderParam::check_param(
        Domain&,
        VectorThree,
        VectorThree,
        Occupancy) {
...
int NumBoundDomainPairsOrderParam::check_param(
        Domain&,
        VectorThree,
        VectorThree,
        Occupancy state) {
...
int NumMisboundDomainPairsOrderParam::check_param(
        Domain&,
        VectorThree,
        VectorThree,
        Occupancy state) {
...
int NumStackedPairsOrderParam::check_param(
        Domain&,
        VectorThree,
        VectorThree,
        Occupancy) {
...
int NumLinearHelicesOrderParam::check_param(
        Domain&,
        VectorThree,
        VectorThree,
        Occupancy) {
...
int NumStackedJunctsOrderParam::check_param(
        Domain&,
        VectorThree,
        VectorThree,
        Occupancy) {
    ...
    for (size_t i {0}; i != level_to_types.size(); i++) {
        ...
        for (size_t j {0}; j != level_to_types[i].size(); j++) {
            ...
            if (type == "Dist" or type == "AdjacentSite") {
                ...
                update_per_domain =
                        ops_file.get_bool_option(i, j, "update_per_domain");
            }
            else if (type == "Sum") {
                ...
                op = new NumStaplesTypeOrderParam {m_origami, staple, label};
            }
            else if (type == "StapleTypeFullyBound") {
                int staple {ops_file.get_int_option(i, j, "staple")};
                op = new StapleTypeFullyBoundOrderParam {
                        m_origami, staple, label};
            }
            else if (type == "NumBoundDomainPairs") {
                op = new NumBoundDomainPairsOrderParam {m_origami, label};
            }
            else if (type == "NumMisboundDomainPairs") {
                op = new NumMisboundDomainPairsOrderParam {m_origami, label};
            }
            else if (type == "NumStackedPairs") {
                op = new NumStackedPairsOrderParam {m_origami, label};
            }
            else if (type == "NumLinearHelices") {
                op = new NumLinearHelicesOrderParam {m_origami, label};
            }
...
void SystemOrderParams::check_one_domain(
        Domain& domain,
        VectorThree pos,
        VectorThree ore,
        Occupancy state) {
