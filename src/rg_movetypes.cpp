...
void CTRGRegrowthMCMovetype::setup_constraints() {
    m_constraintpoints.calculate_constraintpoints(
            m_sel_scaf_doms, m_dir, m_excluded_staples);
    ...
    if (not(m_origami_system.m_cyclic and
            m_sel_scaf_doms.size() == m_origami_system.get_chain(0).size())) {
...
double CTRGRegrowthMCMovetype::set_config(Domain* d, configT c) {
    double delta_e {
            m_origami_system.set_checked_domain_config(*d, c.first, c.second)};
...
double CTRGRegrowthMCMovetype::calc_p_config_open(configT c) {
    double delta_e {
            m_origami_system.check_domain_constraints(*m_d, c.first, c.second)};
...
bool CTRGScaffoldRegrowthMCMovetype::internal_attempt_move() {
    ...
    if (not(m_origami_system.m_cyclic and
            m_regrow_ds.size() == m_origami_system.get_chain(0).size())) {
