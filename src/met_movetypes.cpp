...
bool MetStapleExchangeMCMovetype::staple_insertion_accepted(
        int c_i_ident,
        int num_staple_bd) {
    ...
    double pratio {static_cast<double>(staple_length) * 36 / Ni_new * boltz_factor};
...
bool MetStapleExchangeMCMovetype::staple_deletion_accepted(
        int c_i_ident,
        int num_staple_bd) {
    ...
    size_t staple_length {m_origami_system.m_identities[c_i_ident].size()};
    double pratio {Ni / static_cast<double>(staple_length) / 36 * boltz_factor};
...
bool MetStapleExchangeMCMovetype::insert_staple() {
    ...
    if (m_origami_system.num_staples_of_ident(c_i_ident) ==
        m_max_type_staples) {
        return accepted;
    }
    ...
    accepted = staple_insertion_accepted(
            c_i_ident, num_bound_staple_domains(staple));
    return accepted;
}
