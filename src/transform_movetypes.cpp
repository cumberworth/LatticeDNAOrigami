...
void LinkerRegrowthMCMovetype::write_log_summary(ostream* log_stream) {
    ...
    for (auto l: lengths) {
        *log_stream << "    Number of linker/central domains: " << l.first
                    << "/" << l.second << "\n";
    ...
    *log_stream << "\n";
    for (auto st: staples) {
        *log_stream << "    Number of linker/central staples: " << st.first
                    << "/" << st.second << "\n";
    ...
    *log_stream << "\n";
    for (auto t: transfs) {
        *log_stream << "    Sum/number of displacement/turns: " << t.first
                    << "/" << t.second << "\n";
...
void LinkerRegrowthMCMovetype::reset_segment(
    ...
    for (auto linker_endpoint: {linker1_endpoint, linker2_endpoint}) {
        ...
        if (linker_endpoint != nullptr) {
            m_constraintpoints.add_active_endpoint(
                    linker_endpoint, linker_endpoint->m_pos, seg);
        }
        seg++;
    }
    ...
    for (auto domain: domains) {
        if (domain->m_state != Occupancy::unbound) {
            ...
            if (bound_domain->m_c == m_origami_system.c_scaffold) {
                ...
                if (scan_for_external_scaffold_domain(
...
double LinkerRegrowthMCMovetype::transform_segment(
    ...
    for (int k_i {0}; k_i != m_k; k_i++) {
        ...
        double bfactor {apply_transformation(
                central_domains, disp, center, axis, turns)};
    ...
    for (size_t di {0}; di != central_domains.size(); di++) {
        ...
        if (m_origami_system.position_occupancy(pos) == Occupancy::bound or
            ...
            if (not scaffold_misbinding and new_binding_pair) {
                ...
                m_origami_system.check_domain_constraints(*domain, pos, ore);
...
set<int> ClusteredLinkerRegrowthMCMovetype::select_and_setup_segments(
    ...
    if (p_domain != central_segment.back() and
        p_domain != (*central_segment.back() + 1)) {
...
vector<Domain*> ClusteredLinkerRegrowthMCMovetype::
        linear_select_central_segment() {
    ...
    while (not segment_ended and n_domain != nullptr) {
        if (n_domain->m_state == Occupancy::bound and not segment_started) {
            segment_started = true;
        ...
        while (not segment_ended and n_domain != nullptr) {
            if (n_domain->m_state == Occupancy::bound and not segment_started) {
                ...
                central_segment.push_back(n_domain);
                    ...
                    n_domain->m_state == Occupancy::bound and segment_started) {
                central_segment.push_back(n_domain);
            }
            else if (
                    n_domain->m_state != Occupancy::bound and segment_started) {
...
bool CTCBLinkerRegrowthMCMovetype::internal_attempt_move() {
    ...
    vector<Domain*> central_segment {};
    set<int> staples {
            select_and_setup_segments(linker1, linker2, central_segment)};
    if (m_rejected) {
        return accepted;
    }
    ...
    m_bias *= transform_segment(
            linker1, linker2, central_segment, central_domains);
    if (m_rejected) {
        return accepted;
    }
...
bool CTRGLinkerRegrowthMCMovetype::internal_attempt_move() {
    ...
    unassign_domains();
    unassign_and_save_domains(central_domains);
    m_weight /= revert_transformation(
            linker1, linker2, central_segment, central_domains);
    m_constraintpoints.reset_active_endpoints();
    calc_old_c_opens();
