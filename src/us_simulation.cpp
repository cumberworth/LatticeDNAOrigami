...
long long int USGCMCSimulation::get_current_iter_steps() {
    long long int current_iter_steps {0};
    for (auto& point_count: m_f_i) {
        current_iter_steps += point_count.second;
    }

    return current_iter_steps;
}
...
void SimpleUSGCMCSimulation::update_grids(int) {
    ...
    // Confirm weights add up to 1
    double total_w {0};
    for (auto& point_weight: m_w_i) {
        total_w += point_weight.second;
    }
    if (total_w - 1.0 > 1e-6) {
        throw utility::SimulationMisuse {};
    }
...
MWUSGCMCSimulation::MWUSGCMCSimulation(
        ...
        m_restart_us_filebase {params.m_restart_us_filebase} {
...
PTMWUSGCMCSimulation::PTMWUSGCMCSimulation(
        ...
        m_restart_swap_filename {m_restart_us_filebase + ".swp"} {
    ...
    if (m_rank == m_master_node) {
        if (m_params.m_restart_us_iter == true) {
            ...
            std::stringstream win_to_configi_sstream {win_to_configi_str};
            for (int i {0}; i != m_windows; i++) {
                int configi;
                win_to_configi_sstream >> configi;
                m_win_to_configi[i] = configi;
            }
        }
        else {
            for (auto i {0}; i != m_windows; i++) {
                m_win_to_configi[i] = i;
            }
        }
    }
}
...
void PTMWUSGCMCSimulation::run_swaps(long long int swaps, long long int dur) {
    if (m_iter == 0 and m_params.m_restart_us_iter == true) {
        m_us_sim->m_steps = m_us_sim->get_current_iter_steps();
        cout << "Restarting iteration from step " << m_us_sim->m_steps
             << std::endl;
    }
    else {
        m_us_sim->m_steps = 0;
...
void PTMWUSGCMCSimulation::attempt_exchange(int swap_i) {
    ...
    if (win_to_win[0] != 0) {
        ...
        //     << "\n";
        Chains chains_rec;
        // cout << "Win " << m_rank << ": Recieving chains from " <<
        // win_to_win[0]
        //     << "\n";
...
void PTMWUSGCMCSimulation::initialize_swap_file(InputParameters& params) {
    if (m_rank == m_master_node) {
        string prefix {"_iter-" + std::to_string(m_iter)};
        m_swapfile.open(m_output_filebase + prefix + ".swp");
        for (int rep {0}; rep != m_windows; rep++) {
            m_swapfile << m_win_to_configi[rep] << " ";
        }
        m_swapfile << "\n";
    }
}
